...
class Actor(nn.Module):
	def __init__(self, input_shape, hidden_shape, num_actions, lr, filepath='./saved_models/', file_name='actor.pl', is_her=True):
		...
		self.goal_shape = 3 if is_her else 0

		try:
			os.makedirs(filepath)
		except OSError as e:
			print("Directory '%s' cannot be created" %filepath)

		# Layer 1
		self.fc1 = nn.Linear(self.input_shape + self.goal_shape, hidden_shape[0])
	...
	def save_model(self):
		torch.save(self.state_dict(), self.filepath + self.file_name)

	def load_model(self):
		self.load_state_dict(torch.load(self.filepath + self.file_name))

class Critic(nn.Module):
	def __init__(self, input_shape, hidden_shape, num_actions, lr, l2, filepath='./saved_models/', file_name='critic.pl', is_her=True):
		...
		# Layer 1
		self.fc1 = nn.Linear(self.input_shape + self.goal_shape, self.hidden_shape[0])
