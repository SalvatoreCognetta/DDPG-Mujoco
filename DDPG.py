...
class DDPG(object):
	def __init__(self, input_shape, num_actions, action_space,lr_actor=1e-4, lr_critic=1e-3, l2_critic=1e-2, 
			   gamma=0.99, tau=1e-3, hidden_shape=([400,300]), batch_size=64, rbuffer_size=10**6, HER = True):
		self.gamma = gamma
		self.tau = tau
		...
		self.critic = Critic(self.input_shape, hidden_shape, num_actions, lr_critic, l2_critic, is_her=HER)
		self.critic_target = Critic(self.input_shape, hidden_shape, num_actions, lr_critic, l2_critic, is_her=HER)
		
		# Intialize actor network mu(s|theta^mu) and target network mu'
		self.actor = Actor(self.input_shape, hidden_shape, num_actions, lr_actor, is_her=HER)
		self.actor_target = Actor(self.input_shape, hidden_shape, num_actions, lr_actor, is_her=HER)
		...
		self.HER = HER
		self.replay_buffer = ReplayBuffer(rbuffer_size, input_shape, num_actions, is_her = HER)
	...
	def select_action(self, state, goal=None):
		...
		if self.HER:
			...
			# print('state shape view:', state.view(-1).shape)
		else:
	...
	def save_transition(self, state, action, reward, new_state, done, goal=None):
		if self.HER:
			self.replay_buffer.store_transition(state, action, reward, new_state, done, goal)
		else:
	...
	def learn(self):
		...
		if self.HER:
			state, action, reward, new_state, done, goal = self.replay_buffer.sample_buffer(self.batch_size)
		else:
